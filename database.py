...
def add_agent_and_thread(thread_id, agent_id):
    conn = duckdb.connect("data/agents.db")
    cur = conn.cursor()
...
def add_thread_and_response(thread_id, response):
    conn = duckdb.connect("data/threads.db")
    cur = conn.cursor()
...
if __name__ == "__main__":
    ...
    #add_agent_and_thread(thread_id="food", agent_id="some agent")
    #add_thread_and_response(thread_id="hello", response="hewww")
